...
export function greetRes(app:any) {
    app.message( /(fine)|(good)|(okay)|(sad)|(happy)|(fantastic)|(robot)/ , async({message, say} : {message: {user: string, ts: string}; say: Function}) =>{
        ...
        if ((Number (message.ts)-ts)<8){
            ...
            if((textMsg.includes("fine")&& !textMsg.includes("not")) || (textMsg.includes("good") && !textMsg.includes("not") || (textMsg.includes("am okay") && !textMsg.includes("not")) || textMsg.includes("fantastic"))){
                await say("Good to Hear!");
            }
            if(textMsg.includes("not fine") || textMsg.includes("not good") || textMsg.includes("not okay") || textMsg.includes("sad")){
                await say("I hope you get well soon!");
            }
            if(textMsg.includes("robot")){
                await say("01001111 01010101 01010010 00100000 01010010 01001001 01010011 01000101 00100000 01001001 01010011 00100000 01010011 01001111 01001111 01001110 00100001 00100001")
            }
            }
        }
    )
